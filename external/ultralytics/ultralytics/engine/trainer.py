...
class BaseTrainer:
    ...
    def __init__(self, cfg=DEFAULT_CFG, overrides=None, _callbacks=None):
        ...
        self.metrics = None
        self.plots = {}
        # Ensure seed has a valid value (default to 0 if None)
        seed = self.args.seed if self.args.seed is not None else 0
        init_seeds(seed + 1 + RANK, deterministic=self.args.deterministic)
    ...
    def check_resume(self, overrides):
        ...
        if resume:
            try:
                ...
                # Filter out invalid arguments that are not in DEFAULT_CFG_DICT
                # Convert to dict if it's a Namespace or other type
                if hasattr(ckpt_args, '__dict__'):
                    ckpt_args = vars(ckpt_args)
                elif not isinstance(ckpt_args, dict):
                    ckpt_args = dict(ckpt_args) if hasattr(ckpt_args, 'items') else {}
                
                # Only keep valid YOLO arguments
                valid_keys = set(DEFAULT_CFG_DICT.keys())
                ckpt_args = {k: v for k, v in ckpt_args.items() if k in valid_keys}
                ...
                for k in (
                    ...
                    "device",
                    "close_mosaic",
                    "data",  # Allow data path override when resuming
